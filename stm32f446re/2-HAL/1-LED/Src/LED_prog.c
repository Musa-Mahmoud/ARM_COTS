...
uint8_t LED_u8TurnOn(LED_t *LedConfig)
{
    ...
    else if ((LedConfig->Port > PORTH) || (LedConfig->Port < PORTA) || (LedConfig->Pin > PIN15) || (LedConfig->Pin < PIN0))
    {
        return NOK;
    }
    ...
    {
        Local_u8ErrorState = GPIO_u8SetPinValue(LedConfig->Port, LedConfig->Pin, PIN_HIGH);
    }
    else if (LED_SINK_CONNECTION == LedConfig->ConnectionType)
    {
        Local_u8ErrorState = GPIO_u8SetPinValue(LedConfig->Port, LedConfig->Pin, PIN_LOW);
    }
